...
export async function handleLogin(request, env) {
  ...
  const rows = await query(env,
    `SELECT id, nombre, email, password_hash, rol, urbanizacion_id, activo
  ...
  if (!user.activo) return err(403, 'Usuario desactivado. Contacte al administrador.');
  ...
  const token = await createToken({
    ...
    nombre: user.nombre,
    rol: user.rol,
    urbanizacion_id: user.urbanizacion_id
  }, env.JWT_SECRET);

  return ok({ token, user: { id: user.id, email: user.email, nombre: user.nombre, rol: user.rol, urbanizacion_id: user.urbanizacion_id } });
}
